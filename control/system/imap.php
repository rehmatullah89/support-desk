...
//$EmailsSince      = date('d-M-Y G\:i', time() - 300); // current time - 5 min
//$LatestEmails     = imap_search ($mailbox, 'UNSEEN SINCE ' . $since . '');
//$sSinceDate       = date('d-M-Y'); // current time - 5 min
//$LatestEmails     = imap_search($mailbox, 'BODY "Ticket Reply" UNSEEN SINCE "'.$sSinceDate.'"', SE_UID);
...
if ($mailbox) {
  ...
  for ($i = $loop; $i > 0; $i--) {
    ...
    $filters     = array(
      'txt' => 'no',
      'matches' => array('New Support Ticket Created', 'New Ticket Created','support@support.3-tree.com')
    );
